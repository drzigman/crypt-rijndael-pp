...
sub gf_multiply {
    ...
    #### Left Factor  : ( unpack( "H2", pack( "C", $left_factor ) ) )
    #### Right Factor : ( unpack( "H2", pack( "C", $right_factor ) ) )
    #### Result       : ( unpack( "H2", pack( "C", $PRECOMPUTED->{$left_factor}[$right_factor] ) ) )

    if( !exists $PRECOMPUTED->{$left_factor} ) {
        croak "Left Factor not precomputed";
    }

    return $PRECOMPUTED->{$left_factor}[$right_factor];
}
