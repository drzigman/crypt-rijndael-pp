...
=head1 SYNOPSIS
    ...
    my $cipher = Crypt::Rijndael::PP->new(
        $key, Crypt::Rinjdael::PP::MODE_CBC()
    );
...
=head2 new
    ...
    my $cipher = Crypt::Rinjdael::PP->new(
        $key, Crypt::Rinjdael::PP::MODE_ECB(),
    )

Generates an instance of Crypt::Rinjdael::PP, accepts the key and the mode to use.
...
Crypt::Rinjdael::PP is intended to be fully compatable with Crypt::Rinjdael, and is fully compatable with Crypt::CBC when using 256 bit keys.
...
Special thanks to L<brian d foy|http://www252.pair.com/~comdog/> for his faith and entrusting me with the Crypt::Rijndael module, the L<Houston Perl Mongers|http://houston.pm.org/> for allowing me to present to them my initial implementation, and to L<HostGator|http://www.hostgator.com> and L<BrainStorm Incubator> for providing development resources.
