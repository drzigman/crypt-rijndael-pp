    ...
    for my $num_blocks ( 1, 2, 3 ) {
        subtest "$num_blocks Blocks" => sub {
            test_rijndael_xs_encryption(
                ...
                key   => $KEYS->{128},
                mode  => 'MODE_ECB',
                expected_cipher_text => $CIPHER_TEXT->{ECB}{$num_blocks}{128}, 
            );
    ...
    for my $num_blocks ( 1, 2, 3 ) {
        subtest "$num_blocks Blocks" => sub {
            test_rijndael_xs_encryption(
                ...
                key   => $KEYS->{192},
                mode  => 'MODE_ECB',
                expected_cipher_text => $CIPHER_TEXT->{ECB}{$num_blocks}{192},
            );
            test_rijndael_pp_encryption(
                input => $INPUT_BLOCKS->{$num_blocks},
                key   => $KEYS->{192},
    ...
    for my $num_blocks ( 1, 2, 3 ) {
        subtest "$num_blocks Blocks" => sub {
            test_rijndael_xs_encryption(
                ...
                key   => $KEYS->{256},
                mode  => 'MODE_ECB',
                expected_cipher_text => $CIPHER_TEXT->{ECB}{$num_blocks}{256},
            );
            test_rijndael_pp_encryption(
                input => $INPUT_BLOCKS->{$num_blocks},
                key   => $KEYS->{256},
